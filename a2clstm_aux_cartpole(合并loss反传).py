    ...
    for step in range(STEP):
        ...
        for epoch_i in range(5):
            ...
            for opt_i in range(int(EPISODE_NUM/OPTIM_BATCH)):
                ...
                log_softmax_actions, _, p_batch = actor_network(s_batch, (a_hx, a_cx))
                ...
                p_batch = p_batch.view(-1, 1).squeeze(1)
                t_batch = t_batch.view(-1, 1).squeeze(1)
                criterion = nn.MSELoss()
                aux_loss = criterion(p_batch, t_batch)
                loss = actor_network_loss + aux_loss
                loss.backward()
        ...
        aux_loss_avg = aux_loss_sum.data.numpy() / 25
